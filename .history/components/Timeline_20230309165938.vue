<template>
    <div class="container py-24">
        <div class="flex gap-x-14 relative" ref="scrollEl">
            <div class="bar h-[calc(100vh-100px)] sticky top-[100px] w-5 min-w-[1.25rem] bg-white rounded-full"></div>
...
const { top, height } = useElementBounding(scrollEl);
const { width: w_width, height: w_height } = useWindowSize()

const scrollbarOffsetTop = computed(() => {
    console.log(-top.value / (height.value - document.documentElement.clientHeight))
    return -top.value / (height.value - document.documentElement.clientHeight) * 100;
})

onMounted(() => {
    window.addEventListener('scroll', () => {
        console.log(scrollbarOffsetTop)
    })
})
</script>